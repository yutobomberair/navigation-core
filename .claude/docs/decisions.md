...
## Fork非表示・期限表示・LINE通知導線・カルテ一般化＋適応型テスト（2026-08-03）

ユーザーテストで大量のフィードバックがあり、以下を対応。

### Fork/GitHubツールバーを非表示

`.streamlit/config.toml`に`[client] toolbarMode = "minimal"`を追加。デプロイ後、公開用の
インタビューツールとしてforkを誘うUIを見せたくない、という要望。

### 期限未設定 → 到着予定を保存・表示

ウィザードは元々`deadline`を一切質問しないため、Navigation画面には常に「期限未設定」としか
出せなかった（ユーザーからは「期限を聞かれた覚えがない」という当然の指摘）。ルート生成時に
既に計算している`estimated_arrival`（例:「約6ヶ月後（2027年2月頃）」）を`Goal`に保存し、
Navigationではそちらを表示するように変更。DBマイグレーション: `goals.estimated_arrival`列追加。

### LINE連携導線の見直し

- 友だち追加の案内を、Onboarding最終確認画面だけでなく、目的地入力の直後（`line_connect`
  ステップ）にも前倒しで表示。「後からでもOK」と明示してスキップ可能にした
- 実際に発生した不具合: テスターがコードを送らずに普通にLINEへ話しかけたため、Web側の
  アカウント（`test1`）とは別の新規アカウントが自動生成され、連携されていなかった。
  空の重複アカウントを削除し、正しいコードを送るよう案内して解決。この「先に雑談してしまい
  連携し損ねる」ケースは、コードが既存の別アカウントに紐づいている場合の巻き戻し処理が
  未実装という既知の制限として残っている

### 診断テストの一般化＋適応型（2ラウンド）化

「4択クイズは座学系の目標にしか合わない（起業・筋トレ等に不向き）」「SPI方式で難易度を
動的に変えてほしい」「カルテ（テスト設計）はカテゴリで分岐させず、ある程度一般化できるはず」
という3つの指摘を受けて、`agent/prompts/assessment.py`を全面的に作り直した。

- カテゴリ分類ステップは設けず、**1つの共通プロンプト**の中で、AIがparameterごとに
  「知識確認問題」（語学・専門知識など、客観的な正解がある）か「自己診断形式」（起業・筋トレ
  など、成熟度を選択肢の並びで表現し、最も理想的な選択肢をcorrect_indexとする）かを判断して
  出題する設計にした。分岐ロジックはコード側ではなくプロンプト内の指示のみ
- 1問目（ラウンド1、parameterごとに中程度の難易度で1問）→ユーザー回答→2問目
  （ラウンド2、parameterごとに正誤に応じて難易度を1段階上下）という2段階構成にした
  （`agent/assessment_agent.py::generate_round1`/`generate_round2`）
- Onboardingの「checkpoint」ステップを`checkpoint_round1`/`checkpoint_round2`の2画面に分割
- `assessments`テーブルの`questions`列は、ラウンド1完了時にラウンド2の質問を追記する形で
  更新する（`assessments_repo.update_questions`）。スコアリングは`score()`が
  ラウンド1+2を合わせた全質問に対してそのまま動く（変更不要だった）
